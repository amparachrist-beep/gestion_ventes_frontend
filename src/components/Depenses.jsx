...
import { depenseAPI, boutiqueAPI, profilAPI } from '../api'; // Ajout de profilAPI
import {
  getDepenses,
  saveDepenses,
  saveDepenseOffline
...
export default function Depenses({ isOnline }) {
  ...
  useEffect(() => {
    ...
    if (!checkingPermissions && hasPermission) {
      ...
      // Fallback si hors ligne et que la vérif permission a échoué (car pas de réseau)
      // On tente quand même de charger le local
      loadData();
    }
  ...
  // Si en ligne et pas de permission, on bloque
