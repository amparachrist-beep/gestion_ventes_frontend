...
export default function Produits({ isOnline }) {
  ...
  const [formData, setFormData] = useState({
    nom: '', description: '', prix_achat: '', prix: '', quantite: '', code_barre: '', boutique: ''
  });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.nom || !formData.prix_achat || !formData.prix) return showMessage('❌ Champs obligatoires manquants', 'error');
  ...
  const filteredProduits = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      data = data.filter(p => p.nom.toLowerCase().includes(lower) || (p.code_barre && p.code_barre.includes(lower)));
    }
    return data;
  }, [produits, searchTerm]);
  ...
  return (
    <div className="page-container">
      ...
      <div className="content-wrapper">
        ...
        {/* BOUTON D'ACTION PRINCIPAL (Placé ICI au-dessus de la recherche) */}
        {isOnline && canEdit && (
          <div className="action-section">
            <button className="btn-add-main" onClick={() => { resetForm(); setShowModal(true); }}>
              <Plus size={24} />
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="modal-form">
              <div className="form-group">
                ...
                <input type="text" required autoFocus value={formData.nom} onChange={e => setFormData({...formData, nom: e.target.value})} placeholder="Ex: Savon..." />
              </div>
              <div className="form-row">
                <div className="form-group">
                  <label>Prix Achat (FCFA)</label>
                  <input type="number" required value={formData.prix_achat} onChange={e => setFormData({...formData, prix_achat: e.target.value})} />
                </div>
                <div className="form-group">
              ...
              <div className="form-row">
                <div className="form-group">
                  ...
                  <input type="number" required value={formData.quantite} onChange={e => setFormData({...formData, quantite: e.target.value})} />
                </div>
                <div className="form-group">
                  <label>Code Barre</label>
                  <div className="input-icon-right">
                    <input type="text" value={formData.code_barre} onChange={e => setFormData({...formData, code_barre: e.target.value})} placeholder="Scanner..." />
              ...
              <div className="modal-footer">
                ...
                <button type="submit" className="btn-submit">{editingId ? 'Mettre à jour' : 'Enregistrer'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      <style jsx>{`
        ...
        .back-btn { color: #64748b; padding: 8px; border-radius: 50%; background: #f1f5f9; display: flex; align-items: center; transition: 0.2s; }
        .header h1 { margin: 0; font-size: 1.25rem; font-weight: 700; color: #0f172a; }
        .subtitle { margin: 2px 0 0; font-size: 0.8rem; color: #64748b; }

        /* --- ACTION SECTION (NOUVEAU BOUTON) --- */
        .action-section {
          margin-bottom: 20px;
        }
        .btn-add-main {
        ...
        .search-container input { width: 100%; padding: 14px 40px 14px 44px; border: 1px solid #e2e8f0; border-radius: 12px; font-size: 1rem; background: white; transition: 0.2s; box-shadow: 0 1px 2px rgba(0,0,0,0.05); }
        .search-container input:focus { border-color: #6366f1; outline: none; box-shadow: 0 0 0 3px rgba(99, 102, 241, 0.1); }
        .clear-search { position: absolute; right: 12px; top: 50%; transform: translateY(-50%); background: none; border: none; color: #94a3b8; cursor: pointer; padding: 4px; }

        /* GRID */
        .products-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(280px, 1fr)); gap: 20px; }
        .product-card { background: white; border-radius: 16px; border: 1px solid #f1f5f9; overflow: hidden; transition: transform 0.2s, box-shadow 0.2s; display: flex; flex-direction: column; height: 100%; }
        ...
        .icon-box { width: 48px; height: 48px; background: #f8fafc; border-radius: 12px; display: flex; align-items: center; justify-content: center; color: #64748b; }
        .card-info { padding: 16px; flex: 1; }
        .card-info h3 { margin: 0 0 8px; font-size: 1rem; font-weight: 600; color: #1e293b; line-height: 1.3; }
        .badges { display: flex; flex-wrap: wrap; gap: 6px; }
        .badge-stock { font-size: 0.75rem; padding: 2px 8px; border-radius: 6px; font-weight: 600; }
        .badge-stock.ok { background: #dcfce7; color: #166534; }
        .badge-stock.low { background: #ffedd5; color: #9a3412; }
        .badge-stock.out { background: #fee2e2; color: #991b1b; }
        .badge-code { font-size: 0.7rem; background: #f1f5f9; color: #64748b; padding: 2px 6px; border-radius: 4px; display: flex; align-items: center; gap: 4px; }
        .card-pricing { background: #f8fafc; padding: 12px 16px; border-top: 1px solid #f1f5f9; }
        .price-main { display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px; }
        .price-main .label { font-size: 0.75rem; color: #64748b; text-transform: uppercase; font-weight: 600; }
        .price-main .value { font-size: 1.1rem; font-weight: 700; color: #0f172a; }
        .price-sub { display: flex; justify-content: space-between; font-size: 0.75rem; color: #94a3b8; }
        .profit { color: #10b981; }
        .loss { color: #ef4444; }

        /* FOOTER ACTIONS */
        .card-actions-footer { display: flex; border-top: 1px solid #e2e8f0; }
        .btn-action { flex: 1; display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px; font-size: 0.85rem; font-weight: 600; border: none; cursor: pointer; transition: background 0.2s; }
        .btn-action.edit { background: white; color: #475569; border-right: 1px solid #e2e8f0; }
        .btn-action.edit:hover { background: #f8fafc; color: #1e293b; }
        .btn-action.delete { background: #fff5f5; color: #e53e3e; }
        .btn-action.delete:hover { background: #fee2e2; color: #c53030; }
        ...
        .input-icon-right .icon { position: absolute; right: 10px; top: 50%; transform: translateY(-50%); color: #64748b; pointer-events: none; }
        .modal-footer { margin-top: 8px; display: flex; justify-content: flex-end; gap: 12px; }
        .btn-cancel { background: #f1f5f9; color: #475569; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; }
        .btn-submit { background: #4f46e5; color: white; border: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; cursor: pointer; }

        /* TOAST */
        .toast { position: fixed; bottom: 80px; left: 50%; transform: translateX(-50%); background: #1e293b; color: white; padding: 12px 24px; border-radius: 30px; display: flex; align-items: center; gap: 10px; box-shadow: 0 10px 15px -3px rgba(0,0,0,0.1); animation: slideUp 0.3s ease; z-index: 50; }
        .toast.error { background: #ef4444; }
        @keyframes slideUp { from { transform: translate(-50%, 20px); opacity: 0; } to { transform: translate(-50%, 0); opacity: 1; } }

        /* LOADER & EMPTY */
        .loader-container { padding: 40px; display: flex; justify-content: center; }
        .spinner { width: 32px; height: 32px; border: 3px solid #e2e8f0; border-top-color: #4f46e5; border-radius: 50%; animation: spin 1s linear infinite; }
        .empty-state { text-align: center; color: #94a3b8; padding: 60px 20px; }
        .empty-state h3 { color: #334155; margin: 16px 0 8px; }

        /* MOBILE NAV */
        .mobile-nav { position: fixed; bottom: 0; left: 0; right: 0; background: white; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-around; padding: 12px 0; padding-bottom: max(12px, env(safe-area-inset-bottom)); z-index: 90; }
        ...
        @media (max-width: 480px) {
          ...
          .product-card { flex-direction: row; flex-wrap: wrap; align-items: center; padding: 0; }
          .card-top { padding: 12px; margin-right: 0; width: auto; }
          .icon-box { width: 40px; height: 40px; }
          .card-info { padding: 12px 0; flex: 1; }
          .card-pricing { display: none; }
          .card-actions-footer { width: 100%; }
