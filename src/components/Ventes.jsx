...
export default function Ventes({ isOnline }) {
  ...
  useEffect(() => {
    // Si en ligne, on attend la permission avant de charger
    if (isOnline && !checkingPermissions && hasPermission) {
      loadProfil();
      loadData();
    } else if (!isOnline) {
      // Si hors ligne, on charge les données locales directement
      loadData();
    }
